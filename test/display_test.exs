defmodule TicTacToeTest.Display do
  ...
  test "displays current board" do
    assert TicTacToe.Display.format_board(["1", "2", "3",
      "4", "5", "6",
      "7", "8", "9"])  =~ "1 2 3\n4 5 6\n7 8 9"
  end
  ...
  test "returns welcome message" do
    assert TicTacToe.Display.welcome_introduction =~ "Welcome to Tic Tac Toe"
  end

  test "returns request for name" do
    assert TicTacToe.Display.name_input_request("Player 1") =~ "Player 1, please input your name"
  end

  test "returns request for marker" do
    assert TicTacToe.Display.marker_input_request("Gary") =~ "Gary, please choose a marker"
  end

  test "returns request for turn" do
    assert TicTacToe.Display.turn_input_request("Gary", "x") =~ "Gary's turn with the marker x"
  end

  test "returns message for unavailable cell" do
    assert TicTacToe.Display.unavailable_cell =~ "That position is unavailable"
  end

  test "returns message for draw" do
    assert TicTacToe.Display.draw_message =~ "It's a draw!"
  end

  test "congratulates winner" do
    assert TicTacToe.Display.congratulate_winner("Barry")  =~ "Barry won! Congratulations!"
  end

  test "returns play again request" do
    assert TicTacToe.Display.play_again_input_request =~ "play again?"
  end
